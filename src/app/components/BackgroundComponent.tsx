...
const DataCard = ({ index }) => (
  <div className="bg-indigo-950 text-white rounded-lg shadow-md mb-4 p-4 z-10 w-1/5">
...
const DataTable = () => {
  return (
    <div className="absolute top-0 left-10 flex space-x-4 overflow-x-auto mt-10 z-10">
      {[...Array(5)].map((_, index) => (
        <DataCard key={index} index={index} />
      ))}
    </div>
  );
};
...
export default function BackgroundComponent() {
      ...
      try {
        ...
        if (data.imageUrl !== backgroundImage) {
          ...
          setTimeout(() => {
            ...
            setFade(false);
          }, 1000); // Time for the fade effect to complete
        }
      } catch (error) {
  ...
  return (
    <div className="relative w-full h-[92vh] flex items-center justify-center overflow-hidden bg-gradient-to-b from-gray-900 to-gray-700">
