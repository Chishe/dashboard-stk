...
const AccordionItem: React.FC<{ cat: Cat; isActive: boolean; onClick: () => void }> = ({ cat, isActive, onClick }) => {
  return (
    <div
      className={`relative w-1/3 transition-all duration-500 cursor-pointer overflow-hidden ${isActive ? "w-2/3" : "hover:w-2/3"}`}
      onClick={onClick}
    >
...
const Accordion: React.FC = () => {
  ...
  return (
    <div className="flex h-[92vh]">
      {png.map((cat, index) => (
        <AccordionItem key={index} cat={cat} isActive={activeIndex === index} onClick={() => setActiveIndex(index)} />
      ))}
    </div>
  );
};
